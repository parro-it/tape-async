...
test(`getUsersRoles: /users/id/roles returns success when franchisor owner deletes her own roles`, async (assert) => {
	...
	const dbResult = await db.runQuery(
		`select * from dbo.users_roles where users_id = '` + requesteeUserId + `'`
	);
...
test("use the last test in the file to tearDown the test suite", // you can simplify the test to a single expression. // Here since `tape-async` just expect a promise as return value of your test method
() => startedServer.stop());
